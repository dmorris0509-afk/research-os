...
class GitHubAppAuth:
    ...
    def __init__(self, app_id: str, installation_id: str, private_key: str, *, api_url="https://api.github.com", transport=None):
    ...
    def token(self) -> str:
        if self._token and time.time() < self._expires_at - 60:
            return self._token
        headers = {"Accept": "application/vnd.github+json", "Authorization": f"Bearer {self._jwt()}", "X-GitHub-Api-Version": "2022-11-28"}
        try:
            with httpx.Client(transport=self.transport, timeout=20) as client:
                response = client.post(f"{self.api_url}/app/installations/{self.installation_id}/access_tokens", headers=headers)
